...
public class TaxiMaintenanceScenario {
	...
	public static double MAN_RATE = 1/(30.0*24.0*60);
	public static double REP_RATE = 1/(24.0*60);
	
	public static void main(String[] args) {
		CTMCGenerator ctmc = new CTMCGenerator(new State(NUMBER_OF_TAXIS,0,0,0,0), TaxiMaintenanceScenario::next);
	...
	public static HashMap<State,Double> next(State s) {
		...
		//A taxi needs maintenance
		if(values[T] > 0) {
			int[] newState = Arrays.copyOf(values, values.length);
			newState[T] = newState[T] - 1;
			newState[TM] = newState[TM] + 1;
			toReturn.put(new State(newState), MAN_RATE);
		}
		
		//A taxi is working again
		if(values[TM] > 0) {
			int[] newState = Arrays.copyOf(values, values.length);
			newState[T] = newState[T] + 1;
			newState[TM] = newState[TM] - 1;
			toReturn.put(new State(newState), REP_RATE);
		}
